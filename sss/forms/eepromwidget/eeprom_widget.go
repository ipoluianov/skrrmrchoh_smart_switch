...
func (c *EepromWidget) Compile() {
	...
	for ri := 0; ri < 16; ri++ {
		for i := 0; i < countPerRelay; i++ {
			...
			swonFront := shRelayView.Cell(y, 1)
			swonSwitch := shRelayView.Cell(y, 2)
			swoffFront := shRelayView.Cell(y, 3)
			swoffSwitch := shRelayView.Cell(y, 4)
			rlonFront := shRelayView.Cell(y, 5)
			rlonRelay := shRelayView.Cell(y, 6)
			rloffFront := shRelayView.Cell(y, 7)
			rloffRelay := shRelayView.Cell(y, 8)
