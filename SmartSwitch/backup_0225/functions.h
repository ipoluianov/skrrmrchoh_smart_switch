                ...
                if(switches_state[i] ==1 )//&& switches_double_click[i] <4) //when only "falling edge" occurs
                {
                    ...
                    { 
                        ...
                        for (j=1; j<=(settings[0]); j++) // for every "settings" line
                        {
                            ...
                            {                                   
                                if(eeprom_read_byte(16*j+1) ==2) //see if inverting relay is needed
                                {                                    
                                    if(relays_16 & (1<<eeprom_read_byte(16*j))) //if relay is turned on
                                    {
                                        if(switching_relay_off_prohibited == 0)
                                        {
